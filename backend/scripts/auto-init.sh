...
# 3. Vérifier si c'est une première installation
...
# 4. Déterminer si c'est une première installation
IS_FIRST_INSTALL=false

if [ "$POKEMON_COUNT" -eq 0 ] && [ "$HACKS_COUNT" -eq 0 ] && [ "$USERS_COUNT" -eq 0 ]; then
    IS_FIRST_INSTALL=true
    log_warning "🎉 PREMIÈRE INSTALLATION DÉTECTÉE !"
fi

# 5. Exécuter l'initialisation complète si nécessaire
if [ "$IS_FIRST_INSTALL" = true ]; then
    log_info "🌱 Initialisation complète du projet..."
    
    # Seed complet
    log_info "📦 Installation des données de base..."
    ...
    log_success "✅ Projet déjà initialisé - démarrage normal"
    log_info "  - Les données existent déjà, aucune installation nécessaire"
fi
